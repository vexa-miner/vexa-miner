...
// VexaMiner brand color - #F27121
#undef TFT_ORANGE
#define TFT_ORANGE 0xF384  // RGB(242, 113, 33)
...
void showVexaMinerBootSplash(void) {
  Serial.println("🎨 Displaying VexaMiner boot splash...");
  
  // Set backlight to full brightness immediately
  ledcWrite(0, 255);
  
  // Fill screen with black background
  tft.fillScreen(TFT_BLACK);
  
  // Display "VexaMiner" logo-style text
  tft.setTextDatum(MC_DATUM);  // Middle center alignment
  
  // Draw "Vexa" in white
  tft.setTextSize(3);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString("Vexa", 110, 100);
  
  // Draw "Miner" in orange
  tft.setTextColor(TFT_ORANGE, TFT_BLACK);
  tft.drawString("Miner", 210, 100);
  
  // Draw initializing message (no version number)
  tft.setTextSize(1);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
...
{ 
  ...
  // Test: Set backlight pin directly
  ...
  // *** SHOW VEXAMINER BOOT SPLASH ***
  showVexaMinerBootSplash();
  // ***********************************
...
void esp32_2432S028R_MinerScreen(unsigned long mElapsed)
{
  ...
  render.drawString(data.templates.c_str(), 189-wdtOffset, 20, TFT_WHITE);
  // Best diff
  render.drawString(data.bestDiff.c_str(), 189-wdtOffset, 48, TFT_WHITE);
  // 32Bit shares
  render.setFontSize(18);
  render.drawString(data.completedShares.c_str(), 189-wdtOffset, 76, TFT_WHITE);
  // Hores
  render.setFontSize(14);
  render.setFontColor(TFT_WHITE);
  render.rdrawString(data.timeMining.c_str(), 315-wdtOffset, 104, TFT_WHITE);

  // Valid Blocks
  render.setFontSize(24);
  render.setFontColor(TFT_WHITE);
  render.setAlignment(Align::TopCenter);
  render.drawString(data.valids.c_str(), 290-wdtOffset, 56, TFT_WHITE);

  // Print Temp
  render.setFontSize(10);
  render.setFontColor(0x7BEF);
  render.rdrawString(data.temp.c_str(), 239-wdtOffset, 1, 0x7BEF);
...
{
  // Text-based loading screen with VexaMiner branding
  tft.fillScreen(TFT_BLACK);
  
  tft.setTextDatum(MC_DATUM);
  
  // Draw "VexaMiner" in one line
  tft.setTextSize(3);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString("Vexa", 110, 100);
  ...
  // Draw loading message (smaller)
  tft.setTextSize(1);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString("Loading...", 160, 140, 2);
}
...
// Hidden screens (touch not needed): esp32_2432S028R_ClockScreen, esp32_2432S028R_GlobalHashScreen, esp32_2432S028R_BTCprice
