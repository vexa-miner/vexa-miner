...
static inline void vexa_sha_ll_write_digest_sha256(void *digest_state)
{
...
static inline void vexa_sha_ll_read_digest_swap(void* ptr)
{
  DPORT_INTERRUPT_DISABLE();
...
static inline void vexa_sha_ll_fill_text_block_sha256_swap(const void *input_text)
{
    uint32_t *data_words = (uint32_t *)input_text;
    uint32_t *reg_addr_buf = (uint32_t *)(SHA_TEXT_BASE);
...
static inline void vexa_sha_ll_fill_text_block_sha256_double(const void *input_text)
{
    uint32_t *data_words = (uint32_t *)input_text;
    uint32_t *reg_addr_buf = (uint32_t *)(SHA_TEXT_BASE);
...
IRAM_ATTR void HwShaTest()
{
  ...
  //ESP32S3 39.01KH/s
  test_count = 100000;
  vexaSHA256_context ctx;
  vexa_mids(ctx.digest, s_test_buffer);
  for (int i = 0; i < test_count; ++i)
  {
    vexa_sha256d(&ctx, s_test_buffer+64, hash);
  }
#endif
  ...
  test_count = 100000;
  vexaSHA256_context ctx;
  vexa_mids(&ctx, s_test_buffer);
  vexa_sha256_bake(ctx.digest, s_test_buffer+64, bake);  //15 words
  for (int i = 0; i < test_count; ++i)
  {
    vexa_sha256d_baked(ctx.digest, s_test_buffer+64, bake, hash);
  }
#endif
  ...
  for (int i = 0; i < test_count; ++i)
  {
      ...
      vexa_sha_ll_fill_text_block_sha256(buffer_swap);
      sha_ll_start_block(SHA2_256);

      //sha_hal_hash_block(SHA2_256, s_test_buffer+64, 64/4, false);
      vexa_sha_hal_wait_idle();
      vexa_sha_ll_fill_text_block_sha256(buffer_swap+64);
      sha_ll_continue_block(SHA2_256);

      vexa_sha_hal_wait_idle();
      sha_ll_load(SHA2_256);
      //vexa_sha_ll_read_digest_swap(interResult);

      //sha_hal_hash_block(SHA2_256, interResult, 64/4, true);
      vexa_sha_hal_wait_idle();
      vexa_sha_ll_fill_text_block_sha256_double(inter_swap);
      sha_ll_start_block(SHA2_256);

      vexa_sha_hal_wait_idle();
      sha_ll_load(SHA2_256);
      vexa_sha_ll_read_digest_swap(hash);
  }
#endif
  ...
  for (int i = 0; i < test_count; ++i)
  {
      ...
      //vexa_sha_ll_write_digest_sha256(midstate);
      
      //sha_hal_hash_block(SHA2_256, s_test_buffer+64, 64/4, false);
      //sha_hal_wait_idle();
      vexa_sha_hal_wait_idle();
      //sha_ll_fill_text_block(s_test_buffer+64, 64/4);
      vexa_sha_ll_fill_text_block_sha256(s_test_buffer+64);
      sha_ll_continue_block(SHA2_256);
      ...
      vexa_sha_ll_read_digest(interResult);
      
      //sha_hal_hash_block(SHA2_256, interResult, 64/4, true);
      //sha_hal_wait_idle();
      vexa_sha_hal_wait_idle();
      //sha_ll_fill_text_block(interResult, 64/4);
      vexa_sha_ll_fill_text_block_sha256(interResult);
      sha_ll_start_block(SHA2_256);
