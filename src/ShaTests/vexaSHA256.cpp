...
IRAM_ATTR inline static int Transform_Sha256(vexa_sha256* sha256, const uint8_t* data)
{
    uint32_t S[8], t0, t1;
    int i;
    IRAM_DATA_ATTR uint32_t W[VEXA_BLOCK_SIZE/sizeof(uint32_t)];
...
static int vexa_update(vexa_sha256* sha256, uint8_t* data, uint32_t len)
{
    int ret = 0;
    uint32_t blocksLen;
    uint8_t* local;
    ...
    if (sha256->buffLen > 0) {
        blocksLen = min(len, VEXA_BLOCK_SIZE - sha256->buffLen);
        ...
        if (sha256->buffLen == VEXA_BLOCK_SIZE) {
        
            ByteReverseWords(sha256->buffer, sha256->buffer, VEXA_BLOCK_SIZE);
    ...
    /* process blocks */
    while (len >= VEXA_BLOCK_SIZE) {
        uint32_t* local32 = sha256->buffer;
        XMEMCPY(local32, data, VEXA_BLOCK_SIZE);

        data += VEXA_BLOCK_SIZE;
        len  -= VEXA_BLOCK_SIZE;

        ByteReverseWords(local32, local32, VEXA_BLOCK_SIZE);
...
int vexa_midstate(vexa_sha256* sha256, uint8_t* data, uint32_t len)
{
    int ret = 0;
    uint32_t blocksLen;
    uint8_t* local;
...
IRAM_ATTR int vexa_double_sha2(vexa_sha256* midstate, uint8_t* dataIn, uint8_t* doubleHash)
{
    ...
    ByteReverseWords((uint32_t*)doubleHash, S, VEXA_DIGEST_SIZE);
...
{
    IRAM_DATA_ATTR vexa_sha256 sha256;
    //vexa_sha256 sha256_2;
    int ret = 0;
    uint32_t blocksLen;
    uint8_t* local;
    ...
    local[16] = 0x80; // add 1 
    //ADD final zeros
    XMEMSET(&local[17], 0, 39); //VEXA_PAD_SIZE - sha256.buffLen);
    ...
    XMEMCPY(&local[VEXA_PAD_SIZE], &sha256.hiLen, sizeof(uint32_t));
    XMEMCPY(&local[VEXA_PAD_SIZE + sizeof(uint32_t)], &sha256.loLen, sizeof(uint32_t));
    ...
    IRAM_DATA_ATTR vexa_sha256 secondSha256 = {
    ...
    // ! length ordering dependent on digest endian type ! 
    XMEMCPY(&local[VEXA_PAD_SIZE + sizeof(uint32_t)], &secondSha256.loLen, sizeof(uint32_t));

    XTRANSFORM(&secondSha256, (const uint8_t*)local);

    ByteReverseWords((uint32_t*)doubleHash, secondSha256.digest, VEXA_DIGEST_SIZE);
