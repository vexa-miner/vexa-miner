  ...
  while(true) {
          ...
          case MINING_NOTIFY:         if(parse_mining_notify(line, mJob))
                                      {
                                          ...
                                          vexa_mids(diget_mid, mMiner.bytearray_blockheader);
                                          vexa_sha256_bake(diget_mid, mMiner.bytearray_blockheader+64, bake);
    ...
    } else
    {
      ...
      for (size_t n = 0; n < nonce_vector.size(); ++n)
      {
        ...
        if (vexa_sha256d_baked(diget_mid, mMiner.bytearray_blockheader+64, bake, result->hash))
        {
          result->id = job_pool;
  ...
  while (1)
  {
    ...
    if (job)
    {
      ...
      for (uint32_t n = 0; n < job->nonce_count; ++n)
      {
        ...
        if (vexa_sha256d_baked(job->midstate, job->sha_buffer+64, job->bake, hash))
        {
...
static inline void vexa_sha_ll_fill_text_block_sha256(const void *input_text, uint32_t nonce)
{
...
static inline void vexa_sha_ll_fill_text_block_sha256_inter()
{
  uint32_t *reg_addr_buf = (uint32_t *)(SHA_TEXT_BASE);
...
static inline void vexa_sha_ll_read_digest(void* ptr)
{
  DPORT_INTERRUPT_DISABLE();
...
static inline bool vexa_sha_ll_read_digest_if(void* ptr)
{
  DPORT_INTERRUPT_DISABLE();
...
static inline void vexa_sha_ll_write_digest(void *digest_state)
{
...
static inline void vexa_sha_hal_wait_idle()
{
...
void minerWorkerHw(void * task_id)
{
  ...
  while (1)
  {
    ...
    if (job)
    {
...
#ifdef VALIDATION
      vexa_mids(diget_mid, job->sha_buffer);
      vexa_sha256_bake(diget_mid, job->sha_buffer+64, bake);
#endif
      ...
      {
        //vexa_sha_hal_wait_idle();
        vexa_sha_ll_write_digest(digest_mid);
        //vexa_sha_hal_wait_idle();
        vexa_sha_ll_fill_text_block_sha256(sha_buffer, n);
        //sha_ll_continue_block(SHA2_256);
        REG_WRITE(SHA_CONTINUE_REG, 1);
        
        sha_ll_load(SHA2_256);
        vexa_sha_hal_wait_idle();
        vexa_sha_ll_fill_text_block_sha256_inter();
        //sha_ll_start_block(SHA2_256);
        REG_WRITE(SHA_START_REG, 1);
        sha_ll_load(SHA2_256);
        vexa_sha_hal_wait_idle();
        if (vexa_sha_ll_read_digest_if(hash))
        {
          //Serial.printf("Hw 16bit Share, nonce=0x%X\n", n);
#ifdef VALIDATION
          //Validation
          ((uint32_t*)(job->sha_buffer+64+12))[0] = n;
          vexa_sha256d_baked(diget_mid, job->sha_buffer+64, bake, doubleHash);
          for (int i = 0; i < 32; ++i)
          {
...
static inline bool vexa_sha_ll_read_digest_swap_if(void* ptr)
{
  DPORT_INTERRUPT_DISABLE();
...
static inline void vexa_sha_ll_fill_text_block_sha256(const void *input_text)
{
    uint32_t *data_words = (uint32_t *)input_text;
    uint32_t *reg_addr_buf = (uint32_t *)(SHA_TEXT_BASE);
...
static inline void vexa_sha_ll_fill_text_block_sha256_upper(const void *input_text, uint32_t nonce)
{
    uint32_t *data_words = (uint32_t *)input_text;
    uint32_t *reg_addr_buf = (uint32_t *)(SHA_TEXT_BASE);
...
static inline void vexa_sha_ll_fill_text_block_sha256_double()
{
    uint32_t *reg_addr_buf = (uint32_t *)(SHA_TEXT_BASE);
  ...
  while (1)
  {
    ...
    if (job)
    {
      ...
      for (uint32_t n = 0; n < job->nonce_count; ++n)
      {
        ...
        //vexa_sha_hal_wait_idle();
        vexa_sha_ll_fill_text_block_sha256(sha_buffer);
        sha_ll_start_block(SHA2_256);

        //sha_hal_hash_block(SHA2_256, s_test_buffer+64, 64/4, false);
        vexa_sha_hal_wait_idle();
        vexa_sha_ll_fill_text_block_sha256_upper(sha_buffer+64, job->nonce_start+n);
        sha_ll_continue_block(SHA2_256);
        ...
        vexa_sha_hal_wait_idle();
        vexa_sha_ll_fill_text_block_sha256_double();
        sha_ll_start_block(SHA2_256);

        vexa_sha_hal_wait_idle();
        sha_ll_load(SHA2_256);
        if (vexa_sha_ll_read_digest_swap_if(hash))
        {
          //~5 per second
          double diff_hash = diff_from_target(hash);
          if (diff_hash > result->difficulty)
